...
import { ELECTRICA_CATEGORIES, CATEGORY_BRANDS } from '../constants'
	...
	return (
		<div className="relative w-full mt-10 pt-8 border-t border-slate-100 group">
			<div className="flex items-center justify-between mb-4">
				<h6 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Nuestros Aliados</h6>
			</div>
...
export function ElectricaSection() {
	...
	const initialTab = ELECTRICA_CATEGORIES.find(c => c.id === catParam) || ELECTRICA_CATEGORIES[0]!
	const [activeTab, setActiveTab] = useState(initialTab)

	// Sync tab if URL changes while on the same page
	useEffect(() => {
		if (catParam) {
			const matched = ELECTRICA_CATEGORIES.find(c => c.id === catParam)
						...
						{ELECTRICA_CATEGORIES.map((cat) => (
							<button
								...
								className={cn(
									...
									activeTab.id === cat.id
										? "text-primary bg-white shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)] translate-y-[-2px]"
						...
						<AnimatePresence mode="wait">
							...
							>
								...
								<div className="flex-1 p-8 md:p-12 space-y-8 min-w-0">
									<div>
										<h5 className="text-3xl font-black text-slate-900 mb-6 font-riccione">Aplicaciones</h5>
										<div className={cn(
											"grid gap-8",
