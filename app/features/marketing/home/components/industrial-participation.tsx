			...
			<div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
				...
				<div className="text-center mb-20">
					...
					<motion.h2
						...
						viewport={{ once: true }}
						className="text-4xl md:text-5xl lg:text-7xl font-black text-slate-900 mb-8 tracking-tighter"
					>
					...
					>
						<CarouselContent className="-ml-8">
							{columns.map((column, colIndex) => (
								<CarouselItem key={colIndex} className="pl-8 basis-full sm:basis-1/2 lg:basis-1/4">
									<div className="flex flex-col gap-8">
										{column.map((item, index) => (
											<motion.div
												...
												className="group relative aspect-[3/4] overflow-hidden rounded-2xl bg-slate-900 shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:shadow-red-900/10"
											>
												{/* Base Image */}
												<img
													src={item.image}
												...
												<div className="absolute inset-x-0 bottom-0 p-8 flex flex-col justify-end min-h-[50%]">
													{/* Red accent tag */}
													<div className="mb-4 w-10 h-1 bg-red-600 rounded-full transition-all duration-500 group-hover:w-full" />

													<h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-2 group-hover:text-red-500 transition-colors duration-300">
														{item.title}
													</h3>
												...
												<div className="absolute top-6 right-8 text-white/10 font-black text-4xl select-none group-hover:text-white/20 transition-colors duration-500">
